...
function f2(){
    $.ajax({
        type:'get',
        url:'./data/tsconfig2.json',


        success:function (dataRec) {
            var t=dataRec.data[0].num1;
            if(t<=99 && t>9){
                $(".left_con .num p").text("00"+t)
            }
            else if(t<=9){
                $(".left_con .num p").text("000"+t)
            }
        else if(t<=999 && t>99){
                $(".left_con .num p").text("0"+t)
            }
        else if(t>999 ){
                $(".left_con .num p").text(t)
            };

            $("p.left_p2 span").text(dataRec.data[0].num);
            $("p.zongji_num").text(dataRec.data[0].num3);
            $("p.lixian_num").text(dataRec.data[0].num4)
...
t3 = setInterval($(document).ready(function () {
    ...
    require(
        ...
        function (ec) {
            ...
            var myChart2 = ec.init(document.getElementById('map-wrap'));
                        ...
                        markPoint : {
                            ...
                            tooltip: {
                                ...
                                formatter: function (params, ticket, callback) {
                                    var res = '<div class="e-tooltip"><p class="parking">' + params.data.parking + '</p><p>进出口数量:' + params.data.num + '</br>运维负责人:' + params.data.personName + '</p><p class="right">总车位数:' + params.data.parkingSpace + '</br>联系方式:' + params.data.phone + '</p></div>';
...
// 仅为了模拟异步回调
            ...
            $.ajax({
                type:'get',
                url:'data/tsconfig5.json',
                success:function(data){
                    option.series[1].markPoint["data"]=data.data;
                    myChart2.setOption(option);
                }
            });
        ...
        function (ec) {
            ...
            var option = {
                ...
                legend: {
                    ...
                    itemWidth: 10,
                    itemHeight: 10,
                    textStyle:{color:'rgba(255,255,255,0.7)',fontSize:'14px',fontFamily:'AdobeHeitiStd-Regular'},
                },
                ...
                xAxis : [
                    {
                        ...
                        axisLabel:{
                            show:true,
                            textStyle:{
                                color:'rgba(255,255,255,0.7)',fontSize:'14px',fontFamily:'AdobeHeitiStd-Regular'
                            },
                        },
                        splitLine:{
                            show:true,
