    ...
    eventFn : function(){
    	$(".searchBtn").click(function(){    //  点击查询按钮;
         ...
         _USERNAME=$(".userName").val();
            ...
            LONG_USER_DATA_OBJ = {"userTel":_USERTEL,"userName":_USERNAME,"parkId":CARPARKID,"licensePlateNumber":PLATENO,"operaterName":OPERATOR,"parkingSpaceNumber":PARKINGSPACE,"isExpire":ISEXPIRE};
            gridObj.search(LONG_USER_DATA_OBJ);
            console.log(LONG_USER_DATA_OBJ);
    	});
    	...
    	gridObj = $.fn.bsgrid.init('longUserTable', {
	        ...
	        additionalAfterRenderGrid:function(){
                ...
                for (var i = 0; i < _num3; i++) {
					...
					//鼠标悬停显示完整内容                    
                    $("#longUserTable tbody tr").eq(i).children("td").eq(_num1).on('mouseover',function(){
                    	$(this).attr('title',$(this).html());        //显示写入的内容
                    }) 
                    $("#longUserTable tbody tr").eq(i).children("td").eq(_num2).on('mouseover',function(){
                    	$(this).attr('title',$(this).html());        //显示写入的内容
                    })
